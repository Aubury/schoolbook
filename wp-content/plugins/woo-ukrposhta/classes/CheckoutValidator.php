...
class CheckoutValidator
{
  ...
  public function __construct()
  {
    ...
    add_action('woocommerce_checkout_before_customer_details', array($this, 'add_shipping_method_nonce'));
  ...
  public function add_shipping_method_nonce()
  {
    wp_nonce_field('shipping_method_nonce_action', 'shipping_method_nonce');
  }
...
{
    // Check nonce to secure against CSRF attacks
    if (isset($_POST['shipping_method_nonce'])) {
      $checkout_nonce = sanitize_text_field(wp_unslash($_POST['shipping_method_nonce']));
    
      if (wp_verify_nonce($checkout_nonce, 'shipping_method_nonce_action')) {
          // Check if a shipping method is set
          if (isset($_POST['shipping_method']) && isset($_POST['shipping_method'][0])) {
              ...
              if (strpos($shipping_method, MORKVA_UKRPOSHTA_UP_SHIPPING_NAME) === false) {
                  return; // Skip validation for other shipping methods
              }
          } else {
              return; // No shipping method provided
          }   
      }
    }
    else{
    ...
    // Retrieve billing country or default to 'UA' (Ukraine)
    ...
    // If the billing country is Ukraine, proceed with validation
    if (strpos($billing_country, 'UA') !== false) {
        // Validate city from API data
        $chosen_city = '';
        ...
        $input_city = sanitize_text_field(wp_unslash($_POST[MORKVA_UKRPOSHTA_UP_SHIPPING_NAME . '_city'] ?? $chosen_city));

        // Validate warehouse data from API
        $warehouse_parts = ['Немає відділень'];
        if (isset($_POST[MORKVA_UKRPOSHTA_UP_SHIPPING_NAME . '_postcode_selected'])) {
            $warehouse_parts = explode(' ', sanitize_text_field(wp_unslash($_POST[MORKVA_UKRPOSHTA_UP_SHIPPING_NAME . '_postcode_selected'])));
        }

        $input_postcode = is_array($warehouse_parts) ? sanitize_text_field($warehouse_parts[0]) : '';
        $input_warehouse = sanitize_text_field(wp_unslash($_POST[MORKVA_UKRPOSHTA_UP_SHIPPING_NAME . '_warehouse'] ?? $input_postcode));

        // Validate required fields
        if (
            (!isset($_POST['billing_address_1']) || !isset($_POST['billing_city']) || !isset($_POST['billing_phone'])) &&
  ...
  {
    if (isset($_POST['shipping_method_nonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['shipping_method_nonce'])), 'shipping_method_nonce_action')) 
    {
      add_filter( 'woocommerce_ship_to_different_address_checked', '__return_false' );
      if (isset($data['shipping_method'])) {

        if (preg_match('/^' . MORKVA_UKRPOSHTA_UP_SHIPPING_NAME . '.*/i', $data['shipping_method'][0]) &&  isset($_POST['ship_to_different_address'])
        ) {
  ...
  private function maybeDisableDefaultFields()
  {
    if ( isset($_POST['shipping_method_nonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['shipping_method_nonce'])), 'shipping_method_nonce_action'))
    {
        $shipping_method = isset($_POST['shipping_method'][0]) ? sanitize_text_field(wp_unslash($_POST['shipping_method'][0])) : '';

        return isset($_POST['shipping_method']) &&
            preg_match('/^' . MORKVA_UKRPOSHTA_UP_SHIPPING_NAME . '.*/i', $shipping_method) &&
  ...
  private function getCityByPostcode($postcode='', $lang='UA')
  {
    ...
    if ( is_wp_error( $request ) ) {
      ...
      echo "Щось пішло не так: " . esc_attr($error_message);
      return false;
    }
    ...
    if ( false === simplexml_load_string( $body ) ) {
      ...
      foreach ($errors as $error) {
          echo esc_attr(display_xml_error($error, $xml));
      }
      libxml_clear_errors();
      return false;
    }
