...
function snow_storm_searchpp() {
    ...
    // Define SHORTINIT only if it's not already defined
...
add_action( 'wp_ajax_snow_storm_dismiss_smart_rating', 'dismiss_snow_storm_smart_rating' );
function dismiss_snow_storm_smart_rating() {
	$nonce = isset($_POST['nonce']) ? $_POST['nonce'] : '';
	$action = 'snow_storm_feedback_notification_bar_nonce';

	if (!wp_verify_nonce($nonce, $action)) {
		wp_send_json_error();
	}

	if ( false === get_option( 'snow_storm_smart_rating_dismissed' ) && false === update_option( 'snow_storm_smart_rating_dismissed', false ) ) {
		add_option( 'snow_storm_smart_rating_dismissed', true );
	}
...
function snowstorm_admin_notices() {	
	if ( get_option( 'snow_storm_smart_rating_dismissed', false ) ) {
		return;
	}
	...
	$nonce = wp_create_nonce( 'snow_storm_feedback_notification_bar_nonce' );
	
	if (!empty($showmessage_ratereview)) {
		$rate_url = "https://wordpress.org/support/plugin/snow-storm/reviews/?rate=5#new-post";
		$message = sprintf(__('You have been using %s for some time. Please consider to %s on %s. We appreciate it very much! %s', 'snow-storm'), '<a href="https://wordpress.org/support/plugin/snow-storm/" target="_blank">' . __('Snow Storm', 'snow-storm') . '</a>', '<a href="' . $rate_url . '" target="_blank" class="button"><i class="fa fa-star"></i> ' . __('leave your rating', 'snow-storm') . '</a>', '<a href="https://wordpress.org/support/plugin/snow-storm/reviews/" target="_blank">WordPress.org</a>', '<button type="button" class="button my-custom-dismiss-button" data-nonce="' . $nonce .'">' . __('Dismiss forever', 'one-click-ssl') . '</button>');
		$message .= "
					<script>
					jQuery('.my-custom-dismiss-button').on('click', function(e) {
						e.preventDefault();
					var nonce =jQuery(this).attr('data-nonce');
					jQuery.post(ajaxurl,{action:'snow_storm_dismiss_smart_rating',nonce:nonce})
						jQuery('.notice-snow-storm .notice-dismiss').trigger('click');
					});
					</script>";
