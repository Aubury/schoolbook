...
if ( ! class_exists( 'AWS_Integrations' ) ) :
    ...
    class AWS_Integrations {
        ...
        public function __construct() {
            ...
            if ( function_exists( 'gp_npwp_set_non_purchasable' ) ) {
                add_filter( 'aws_search_pre_filter_products', array( $this, 'non_purchasable_woocommerce_products_filter' ), 10, 2 );
            }
            ...
            // Add custom fields for search
            add_filter( 'aws_meta_keys_include', array( $this, 'filter_meta_keys_include' ) );
            ...
            if ( 'OceanWP' === $this->current_theme ) {
                add_action( 'wp_enqueue_scripts', array( $this,  'oceanwp_wp_enqueue_scripts' ), 9999999 );
            }
            ...
            //  BeRocket Min and Max Quantity for WooCommerce plugin
            if ( defined( 'BeRocket_MM_Quantity_version' ) ) {
                add_filter( 'aws_search_pre_filter_products', array( $this, 'berocket_min_and_max_quantity' ), 10, 2 );
            }
        ...
        public function includes() {
            ...
            if ( defined( 'WP_CLI' ) && WP_CLI ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-wp-cli.php' );
            }

            // Getenberg block
            if ( function_exists( 'register_block_type' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/gutenberg/class-aws-gutenberg-init.php' );
            }

            // Elementor plugin widget
            if ( defined( 'ELEMENTOR_VERSION' ) || defined( 'ELEMENTOR_PRO_VERSION' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/elementor-widget/class-elementor-aws-init.php' );
            }
            ...
            if ( class_exists( 'FLBuilder' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/bb-aws-search/class-aws-bb-module.php' );
            }

            // ACF plugin fields class
            if ( class_exists('ACF') ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-acf.php' );
                AWS_ACF::instance();
            }

            // WCFM - WooCommerce Multivendor Marketplace
            if ( class_exists( 'WCFMmp' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-wcfm.php' );
                AWS_WCFM::instance();
            }
            ...
            if ( defined( 'WOOF_PLUGIN_NAME' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-woof-filter.php' );
            }

            // Ultimate Member plugin hide certain products
            if ( class_exists( 'UM_Functions' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-um.php' );
            }

            // Wholesale plugin
            if ( class_exists( 'WooCommerceWholeSalePrices' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-wholesale.php' );
            }

            // B2BKing plugin
            if ( class_exists( 'B2bking' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-b2bking.php' );
            }

            // BeRocket WooCommerce Advanced Product Labels plugin
            if ( class_exists( 'BeRocket_products_label' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-berocket-labels.php' );
            }

            // Advanced Woo Labels plugin
            if ( function_exists( 'AWL' ) || function_exists( 'AWL_PRO' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-awl.php' );
            }

            // Products Visibility by User Roles plugin
            if ( class_exists( 'Addify_Products_Visibility' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-products-visibility.php' );
            }

            // WooCommerce Products Visibility
            if ( class_exists('WooCommerce_Products_Visibility') ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-wcpv.php' );
            }

            // WPBakery plugin
            if ( defined( 'WPB_VC_VERSION' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-wpbakery.php' );
            }

            // WPML plugin
            if ( defined( 'ICL_SITEPRESS_VERSION' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-wpml.php' );
            }

            // WooCommerce Product Filter by WooBeWoo
            if ( defined( 'WPF_PLUG_NAME' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-woobewoo-filters.php' );
            }

            if ( class_exists( 'WC_PPC_Util' ) || class_exists( '\Barn2\Plugin\WC_Protected_Categories\Util' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-barn2-protected-categories.php' );
            }

            // WooCommerce Product Table plugin by Barn2
            if ( class_exists( 'Barn2\Plugin\WC_Product_Table\Product_Table' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-barn2-product-table.php' );
            }

            // FacetWP plugin
            if ( class_exists( 'FacetWP' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-facetwp.php' );
            }

            // Dokan plugin
            if ( class_exists( 'WeDevs_Dokan' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-dokan.php' );
            }

            // WC Marketplace - https://wc-marketplace.com/
            if ( defined( 'WCMp_PLUGIN_VERSION' ) || defined( 'MVX_PLUGIN_VERSION' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-multivendorx.php' );
            }
            ...
            if ( class_exists('YIKES_Custom_Product_Tabs') || class_exists( 'YIKES_Custom_Product_Tabs_Pro' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-custom-tabs.php' );
            }

            // WooCommerce Brands
            if ( class_exists( 'WC_Brands' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-woo-brands.php' );
            }

            // Perfect Brands for WooCommerce
            if ( defined( 'PWB_PLUGIN_VERSION' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-pwb.php' );
            }

            // Astra theme
            if ( 'Astra' === $this->current_theme ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-astra.php' );
            }

            // Flatsome theme
            if ( 'Flatsome' === $this->current_theme ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-flatsome.php' );
            }

            // Woodmart theme
            if ( 'Woodmart' === $this->current_theme ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-woodmart.php' );
            }

            // The7 theme
            if ( 'The7' === $this->current_theme ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-the7.php' );
            }

            // Bricks Builder theme
            if ( class_exists( '\Bricks\Elements' ) ) {
                add_action( 'init', function() {
                    \Bricks\Elements::register_element( AWS_PRO_DIR . '/includes/modules/class-aws-bricks-builder.php' );
                }, 11 );
            }

            // Avada theme
            if ( class_exists( 'Avada' ) || 'Avada' === $this->current_theme ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-avada.php' );
            }

            // GeneratePress theme
            if ( 'GeneratePress' === $this->current_theme ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-generatepress.php' );
            }

            // Product Filters for WooCommerce
            if ( defined( 'WC_PRODUCT_FILTER_VERSION' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-pfw.php' );
            }

            // WooCommerce Product Bundles
            if ( class_exists( 'WC_Bundles' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-product-bundles.php' );
            }

            // Shopengine plugin
            if ( class_exists( 'ShopEngine' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-shopengine.php' );
            }

            // BeRocket WooCommerce AJAX Products Filter
            if ( defined( 'BeRocket_AJAX_filters_version' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-berocket-filters.php' );
            }

            // WooCommerce Memberships
            if ( class_exists( 'WC_Memberships' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-woo-memberships.php' );
            }

            // WooCommerce Show Single Variations by Iconic
            if ( class_exists('Iconic_WSSV') || class_exists('JCK_WSSV') ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-single-variations.php' );
            }

            // YITH WooCommerce Ajax Product Filter
            if ( defined( 'YITH_WCAN' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-yith-wcan.php' );
            }

            // Filter Everything
            if ( class_exists( 'FlrtFilter' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-filter-everything.php' );
            }

            //  EAN for WooCommerce by WPFactory
            if ( class_exists( 'Alg_WC_EAN' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-alg-wc-ean.php' );
            }

            // Breakdance builder
            if ( defined( 'BREAKDANCE_PLUGIN_URL' ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-breakdance.php' );
            }

            // WC Vendors plugin
            if ( in_array( 'wc-vendors/class-wc-vendors.php', $this->active_plugins ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-wcvendors.php' );
            }
            ...
            if ( in_array( 'jet-blocks/jet-blocks.php', $this->active_plugins ) || in_array( 'jet-elements/jet-elements.php', $this->active_plugins ) || in_array( 'jet-woo-builder/jet-woo-builder.php', $this->active_plugins ) ) {
                include_once( AWS_PRO_DIR . '/includes/modules/class-aws-crocoblock.php' );
            }
         ...
         * Non-Purchasable WooCommerce Products
         */
        public function non_purchasable_woocommerce_products_filter( $products_array, $data ) {
            ...
            foreach( $products_array as $key => $pr_arr ) {
                ...
                if ( isset( $pr_arr['id'] ) ) {
                    $not_buyable = get_post_meta( $pr_arr['id'], '_not_buyable', true );
                    if ( $not_buyable === 'yes' ) {
                        $pr_arr['add_to_cart'] = '';
                    }
                }
        ...
        /*
         * Woostify theme markup for seamless js integration
         */
        ...
        */
        ...
        public function claue_wp_head() { ?>
            <style>
                ...
                #jas-header .aws-container .aws-search-form .aws-main-filter .aws-main-filter__current,
                #jas-header .aws-container .aws-search-form,
                #jas-header .aws-container .aws-search-field,
        ...
        */
        ...
        public function gecko_wp_head() { ?>
            <style>
                ...
                #jas-wrapper .aws-container.header__search .aws-search-form .aws-form-btn {
                    background: transparent;
                    border: none;
                    border-bottom: 1px solid rgba(255, 255, 255, .1);
                }
                #jas-wrapper .aws-container.header__search .aws-search-form .aws-main-filter .aws-main-filter__current {
                    color: #555;
                }
                ...
                window.addEventListener('load', function() {
                    ...
                    function aws_results_layout( styles, options ) {
                        ...
                        if ( options.form.closest('#jas-wrapper').length > 0 && jQuery('.aws-container.header__search').is(':visible') ) {
                            ...
                            var offsetTop = parseInt( realForm.css('top') );
                            var offsetLeft = parseInt( realForm.css('left') );
                            var bodyOffset = jQuery('body').offset();
                            var width = realForm.outerWidth();
                            var bodyWidth = jQuery('body').outerWidth();
                            styles.top = offsetTop + form.find('.aws-search-field').innerHeight();
                            if ( typeof styles.right !== 'undefined' ) {
                                styles.right = bodyWidth - offsetLeft + bodyOffset.left - width + 130;
                            }
                    ...
                    AwsHooks.add_filter( "aws_results_layout", aws_results_layout );
                    AwsHooks.add_filter( "aws_main_filter_layout", aws_results_layout );
                }, false);
            </script>
        ...
        public function vandana_wp_head() { ?>
            <style>
                .header-search .header-search-wrap .aws-container .aws-search-form {
                    background: transparent;
                }
                ...
                .header-search .header-search-wrap .aws-container .aws-search-form .aws-main-filter .aws-main-filter__current {
                    font-size: 16px;
                    color: #fff;
                }
            </style>
        ...
        /*
         * Oxygen theme: fix images display for product variations
         */
        public function oxygen_post_thumbnail_id( $thumbnail_id, $post ) {
        ...
        /*
         * Enable custom fields for search
         */
        public function filter_meta_keys_include( $include_meta ) {
            $include_meta[] = '_kad_tab_';
            return $include_meta;
        }
        ...
        public function jupiter_head_action() { ?>
            ...
            <style>
                ...
                .mk-fullscreen-search-overlay .aws-container .aws-suggest {
                    display: none;
                }

                .mk-fullscreen-search-overlay .aws-container .aws-search-form .aws-main-filter .aws-main-filter__current {
                    color: #555;
                }
        ...
        public function storefront_footer_action() {
            ...
            $mobile_screen = AWS_PRO()->get_settings( 'mobile_overlay', $form_id );
            ?>
        ...
        public function head_js_integration() {
            ...
            /**
             * Filter seamless integrations default form id
             * @since 1.82
             * @param int $form_id Id of search form
             */
            $form_id = apply_filters( 'aws_js_seamless_form_id', false );

            if ( $form_id && is_int( $form_id ) ) {
                $form_args = array( 'id' => $form_id );
            }

            $forms_selector = implode( ',', $forms );

            $form_html = str_replace( 'aws-container', 'aws-container aws-js-seamless', aws_get_search_form( false, $form_args ) );
        ...
        /*
         * OceanWP fix cart refresh
         */
        public function oceanwp_wp_enqueue_scripts() {
         ...
         * BeRocket Min and Max Quantity for WooCommerce plugin
         */
        public function berocket_min_and_max_quantity( $products_array, $data ) {

            foreach( $products_array as $key => $pr_arr ) {

                if ( isset( $pr_arr['add_to_cart'] ) && is_array( $pr_arr['add_to_cart'] ) ) {

                    $min_qty = get_post_meta( $pr_arr['id'], 'min_quantity', true );
                    $max_qty = get_post_meta( $pr_arr['id'], 'max_quantity', true );

                    if ( $min_qty ) {
                        $products_array[$key]['add_to_cart']['quantity_min'] = $min_qty;
                    }

                    if ( $max_qty ) {
                        $products_array[$key]['add_to_cart']['quantity_max'] = $max_qty;
                    }
