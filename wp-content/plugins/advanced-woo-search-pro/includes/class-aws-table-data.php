...
if ( ! class_exists( 'AWS_Table_Data' ) ) :
    ...
    class AWS_Table_Data {
        ...
        public function scrap_data() {
            ...
            $brand_array = $this->options['index']['brand'] ? AWS_Helpers::get_terms_array( $data['id'] , 'product_brand', 'brand' ) : false;
            ...
            // Product brands
            if ( $brand_array && ! empty( $brand_array ) ) {
                foreach( $brand_array as $brand_source => $brand_terms ) {
                    $data['terms'][$brand_source] = $this->extract_terms( $brand_terms, 'brand' );
                }
            }
