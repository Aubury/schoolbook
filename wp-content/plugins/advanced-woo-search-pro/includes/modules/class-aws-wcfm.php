...
if ( ! class_exists( 'AWS_WCFM' ) ) :
    ...
    class AWS_WCFM {
        ...
        public function __construct() {
            ...
            add_filter( 'aws_users_search_query', array( $this, 'wcfm_users_search_query' ), 1, 3 );
                ...
                foreach( $result_array as $user_id => $user_params ) {
                    ...
                    if ( in_array( 'dc_vendor', $user_roles ) || in_array( 'wcfm_vendor', $user_roles ) ) {
                        ...
                        if ( function_exists( 'wcfm_get_vendor_store_name' ) ) {
                            $result_array[$user_id][0]['name'] = wcfm_get_vendor_store_name( absint( $user_id ) );
                        }
        ...
        */
        ...
        /*
         * WCFM - Update SQL query and add meta fields
         */
        public function wcfm_users_search_query( $sql, $user_roles, $search_query ) {
            $sql = preg_replace( '/meta_key IN \(([\s\S]*?)\)/i', "meta_key IN ( $1, 'wcfmmp_store_name', 'store_name' )", $sql );
            return $sql;
        }
