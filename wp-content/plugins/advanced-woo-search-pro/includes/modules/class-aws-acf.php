...
if ( ! class_exists( 'AWS_ACF' ) ) :
    ...
    class AWS_ACF {
        ...
        public function product_acf( $condition_rule ) {
            ...
            $relation = ( $condition_rule['operator'] === 'not_equal' || $condition_rule['operator'] === 'not_contains' ) ? 'NOT IN' : 'IN';
            $string = '';
            ...
            if ( is_array( $acf_inner_fields ) && ! empty( $acf_inner_fields ) ) {
                ...
                if ( $value ) {
                    ...
                    $operator = sprintf( "meta_value = '%s'", $value );

                    switch ( $condition_rule['operator'] ) {
                        case 'contains':
                        case 'not_contains':
                            $operator = sprintf( "meta_value LIKE '%s'", '%' . $wpdb->esc_like( $value ) . '%' );
                            break;
                        case 'greater':
                            $operator = sprintf( "meta_value >= '%s'", $value );
                            break;
                    ...
                    $string = "( id {$relation} (
                        SELECT post_id
                        FROM $wpdb->postmeta
                        WHERE meta_key = '{$acf_inner}' AND {$operator}
                    ))";
