        ...
        private function register_wpml_translations( $params ) {
            ...
            foreach( $params as $search_instance_num => $search_instance_settings ) {
                ...
                foreach ( $options_to_reg as $key => $option ) {
                    if ( isset( $search_instance_settings[$key] ) ) {
                        icl_register_string( 'aws', $key . '_' . $search_instance_num, $search_instance_settings[$key] );
                    }
                }

                if ( isset( $search_instance_settings['quick_filters'] ) ) {
                    foreach( $search_instance_settings['quick_filters'] as $filter_num => $filter_settings ) {
                        if ( isset( $filter_settings['item_name'] ) ) {
                            icl_register_string( 'aws', 'filter_name' . '_' . $search_instance_num . '_' . $filter_num, $filter_settings['item_name'] );
                        }
                    }
                }
