...
class AWS_PRO_Versions {
    ...
    public function setup() {
        ...
        $reindex_version = ( function_exists('AWS_PRO') && AWS_PRO()->option_vars )
            ? AWS_PRO()->option_vars->get_reindex_version()
            : get_option( 'aws_pro_reindex_version' );
        ...
        if ( $current_version ) {
            ...
            if ( version_compare( $current_version, '2.63', '<' ) ) {
                ...
                if ( $settings ) {
                    ...
                    foreach( $settings as $search_instance_num => $search_instance_settings ) {
                        if ( isset( $search_instance_settings['filters'] ) ) {
                            foreach ($search_instance_settings['filters'] as $filter_num => $filter_settings) {
                                if ( ! isset( $filter_settings['pages_results_num'] )  ) {
                                    $settings[$search_instance_num]['filters'][$filter_num]['pages_results_num'] = 10;
                                }
                            }
                        }
                    }
            ...
            if ( version_compare( $current_version, '2.76', '<' ) ) {
                ...
                if ( $common_opts ) {
                    ...
                    if ( ! isset( $common_opts['index_shortcodes'] ) ) {
                        $common_opts['index_shortcodes'] = 'true';
                        update_option( 'aws_pro_common_opts', $common_opts );
                    }
            ...
            if ( version_compare( $current_version, '3.37', '<' ) ) {
                    ...
                    foreach( $settings as $search_instance_num => $search_instance_settings ) {
                        if ( isset( $search_instance_settings['filters'] ) ) {
                            foreach ($search_instance_settings['filters'] as $filter_num => $filter_settings) {
                                ...
                                if ( ! isset( $filter_settings['show_brand'] )  ) {
                                    $settings[$search_instance_num]['filters'][$filter_num]['show_brand'] = 'false';
                                }

                                if ( isset( $filter_settings['search_in'] ) ) {
                                    $brand_search_enabled = isset( $filter_settings['search_in']['tax'] ) && $filter_settings['search_in']['tax'] && isset( $filter_settings['search_in_tax'] ) && isset( $filter_settings['search_in_tax']['tax_product_brand'] ) && $filter_settings['search_in_tax']['tax_product_brand'] ? 1 : 0;
                                    $settings[$search_instance_num]['filters'][$filter_num]['search_in']['brand'] = $brand_search_enabled;
                                }

                                if ( isset( $filter_settings['search_archives'] ) ) {
                                    $archive_brand_search = isset( $filter_settings['search_archives']['archive_tax'] ) && $filter_settings['search_archives']['archive_tax'] && isset( $filter_settings['search_archives_tax'] ) && isset( $filter_settings['search_archives_tax']['product_brand'] ) && $filter_settings['search_archives_tax']['product_brand'] ? 1 : 0;
                                    $settings[$search_instance_num]['filters'][$filter_num]['search_archives']['archive_brand'] = $archive_brand_search;
                                }
                ...
                if ( $common_opts ) {
                    ...
                    if ( isset( $common_opts['index_sources'] ) ) {
                        ...
                        $brand_index_enabled = isset( $common_opts['index_sources']['tax'] ) && $common_opts['index_sources']['tax'] && isset( $common_opts['index_sources_tax'] ) && isset( $common_opts['index_sources_tax']['tax_product_brand'] ) && $common_opts['index_sources_tax']['tax_product_brand'] ? 1 : 0;
                        $common_opts['index_sources']['brand'] = $brand_index_enabled;
            ...
            if ( version_compare( $current_version, '3.41', '<' ) ) {

                $common_opts = get_option( 'aws_pro_common_opts' );

                if ( $common_opts ) {

                    if ( ! isset( $common_opts['stopwords'] ) ) {
                        $stopwords = get_option( 'aws_pro_stopwords' );
                        if ( $stopwords ) {
                            $common_opts['stopwords'] = $stopwords;
                            update_option( 'aws_pro_common_opts', $common_opts );
                        }
                    }

                    if ( ! isset( $common_opts['synonyms'] ) ) {
                        $synonyms = get_option( 'aws_pro_synonyms' );
                        if ( $synonyms ) {
                            $common_opts['synonyms'] = $synonyms;
                            update_option( 'aws_pro_common_opts', $common_opts );
                        }
                    }
                    ...
                    foreach( $settings as $search_instance_num => $search_instance_settings ) {
                        ...
                        if ( isset( $search_instance_settings['filters'] ) ) {
                            foreach ($search_instance_settings['filters'] as $filter_num => $filter_settings) {
                                ...
                                if ( isset( $filter_settings['search_in'] ) && ! isset( $search_instance_settings['search_in'] ) ) {
                                    $settings[$search_instance_num]['search_in'] = $filter_settings['search_in'];
                                }

                                if ( isset( $filter_settings['search_in_attr'] ) && ! isset( $search_instance_settings['search_in_attr'] ) ) {
                                    $settings[$search_instance_num]['search_in_attr'] = $filter_settings['search_in_attr'];
                                }

                                if ( isset( $filter_settings['search_in_tax'] ) && ! isset( $search_instance_settings['search_in_tax'] ) ) {
                                    $settings[$search_instance_num]['search_in_tax'] = $filter_settings['search_in_tax'];
                                }

                                if ( isset( $filter_settings['search_in_meta'] ) && ! isset( $search_instance_settings['search_in_meta'] ) ) {
                                    $settings[$search_instance_num]['search_in_meta'] = $filter_settings['search_in_meta'];
                                }
                        ...
                        $image_source = $settings[$search_instance_num]['image_source'];
                        if ( is_string( $image_source ) ) {
                            ...
                            $image_sources_array = explode( ',', $image_source );

                            if ( is_array( $image_sources_array ) && ! empty( $image_sources_array ) ) {

                                $new_array = array();
                                foreach ( $image_sources_array as $image_source ) {
                                    $new_array[$image_source] = 1;
                                }

                                foreach ( $default_values as $default_val_id => $default_val ) {
                                    if ( ! isset( $new_array[$default_val_id] ) ) {
                                        $new_array[$default_val_id] = 0;
                                    }
                                }

                                $settings[$search_instance_num]['image_source'] = $new_array;

                            } else {
                                $settings[$search_instance_num]['image_source'] = $default_values;
                            }
                        ...
                        if ( isset( $search_instance_settings['filters'] ) ) {
                            foreach ($search_instance_settings['filters'] as $filter_num => $filter_settings) {
                                ...
                                if ( isset( $filter_settings['search_archives'] )  ) {
                                    ...
                                    $available_archives = array( 'archive_category', 'archive_tag', 'archive_brand', 'archive_tax', 'archive_attr', 'archive_users' );

                                    foreach ( $available_archives as $search_archive_name ) {

                                        $val = '0';
                                        if ( isset( $filter_settings['search_archives'][$search_archive_name] ) && $filter_settings['search_archives'][$search_archive_name] ) {
                                            if ( is_array( $filter_settings['search_archives'][$search_archive_name] ) && isset( $filter_settings['search_archives'][$search_archive_name]['value'] ) ) {
                                                break;
                                            }
                                            $val = '1';
                                        }
                                ...
                                if ( isset( $filter_settings['search_archives_tax'] ) && is_array( $filter_settings['search_archives_tax'] ) ) {
                                    foreach ( $filter_settings['search_archives_tax'] as $search_archives_tax_name => $search_archives_tax_val ) {
                                        if ( $search_archives_tax_val && isset( $search_archives_new['archive_tax'] ) ) {
                                            $field_name = 'tax_' . $search_archives_tax_name;
                                            $search_archives_new['archive_tax']['fields'][$field_name]['value'] = '1';
                                        }
                                    }
                                }

                                if ( isset( $filter_settings['search_archives_attr'] ) && is_array( $filter_settings['search_archives_attr'] ) ) {
                                    foreach ( $filter_settings['search_archives_attr'] as $search_archives_attr_name => $search_archives_attr_val ) {
                                        if ( $search_archives_attr_val && isset( $search_archives_new['archive_attr'] ) ) {
                                            $field_name = 'attr_' . $search_archives_attr_name;
                                            $search_archives_new['archive_attr']['fields'][$field_name]['value'] = '1';
                                        }
                                    }
                                }

                                if ( isset( $filter_settings['search_archives_users'] ) && is_array( $filter_settings['search_archives_users'] ) ) {
                                    foreach ( $filter_settings['search_archives_users'] as $search_archives_users_name => $search_archives_users_val ) {
                                        if ( $search_archives_users_val && isset( $search_archives_new['archive_users'] ) ) {
                                            $field_name = $search_archives_users_name;
                                            $search_archives_new['archive_users']['fields'][$field_name]['value'] = '1';
                                        }
                                    }
                                }

                                $settings[$search_instance_num]['filters'][$filter_num]['search_archives'] = $search_archives_new;
            ...
            if ( version_compare( $current_version, '3.45', '<' ) ) {
                ...
                if ( $settings ) {
                    ...
                    foreach( $settings as $search_instance_num => $search_instance_settings ) {
                        ...
                        if ( isset( $search_instance_settings['search_in'] )  ) {
                            ...
                            $available_search_in = array( 'title', 'content', 'sku', 'gtin', 'excerpt', 'brand', 'category', 'tag', 'id', 'attr', 'tax', 'meta' );

                            foreach ( $available_search_in as $search_in_name ) {

                                $val = '0';
                                if ( isset( $search_instance_settings['search_in'][$search_in_name] ) && $search_instance_settings['search_in'][$search_in_name] ) {
                                    if ( is_array( $search_instance_settings['search_in'][$search_in_name] ) && isset( $search_instance_settings['search_in'][$search_in_name]['value'] ) ) {
                                        break;
                                    }
                                    $val = '1';
                                }
                        ...
                        if ( isset( $search_instance_settings['search_in_attr'] ) && is_array( $search_instance_settings['search_in_attr'] ) ) {
                            foreach ( $search_instance_settings['search_in_attr'] as $search_in_attr_name => $search_in_attr_val ) {
                                if ( $search_in_attr_val && isset( $search_in_new['attr'] ) ) {
                                    $search_in_new['attr']['fields'][$search_in_attr_name]['value'] = '1';
                                }
                            }
                        }

                        if ( isset( $search_instance_settings['search_in_tax'] ) && is_array( $search_instance_settings['search_in_tax'] ) ) {
                            foreach ( $search_instance_settings['search_in_tax'] as $search_in_tax_name => $search_in_tax_val ) {
                                if ( $search_in_tax_val && isset( $search_in_new['tax'] ) ) {
                                    $search_in_new['tax']['fields'][$search_in_tax_name]['value'] = '1';
                                }
                            }
                        }

                        if ( isset( $search_instance_settings['search_in_meta'] ) && is_array( $search_instance_settings['search_in_meta'] ) ) {
                            foreach ( $search_instance_settings['search_in_meta'] as $search_in_meta_name => $search_in_meta_val ) {
                                if ( $search_in_meta_val && isset( $search_in_new['meta'] ) ) {
                                    $search_in_new['meta']['fields'][$search_in_meta_name]['value'] = '1';
                                }
                            }
                        }
                        $settings[$search_instance_num]['search_in'] = $search_in_new;
                ...
                if ( $common_opts ) {
                    ...
                    if ( isset( $common_opts['index_sources'] )  ) {
                        ...
                        $available_index_sources = array( 'title', 'content', 'sku', 'gtin', 'excerpt', 'brand', 'category', 'tag', 'id', 'attr', 'tax', 'meta' );

                        foreach ( $available_index_sources as $search_source_name ) {

                            $val = '0';
                            if ( isset( $common_opts['index_sources'][$search_source_name] ) && $common_opts['index_sources'][$search_source_name] ) {
                                if ( is_array( $common_opts['index_sources'][$search_source_name] ) && isset( $common_opts['index_sources'][$search_source_name]['value'] ) ) {
                                    break;
                                }
                                $val = '1';
                            }
                    ...
                    if ( isset( $common_opts['index_sources_attr'] ) && is_array( $common_opts['index_sources_attr'] ) ) {
                        foreach ( $common_opts['index_sources_attr'] as $index_sources_attr_name => $index_sources_attr_val ) {
                            if ( $index_sources_attr_val && isset( $index_new['attr'] ) ) {
                                $index_new['attr']['fields'][$index_sources_attr_name]['value'] = '1';
                            }
                        }
                    }

                    if ( isset( $common_opts['index_sources_tax'] ) && is_array( $common_opts['index_sources_tax'] ) ) {
                        foreach ( $common_opts['index_sources_tax'] as $index_sources_tax_name => $index_sources_tax_val ) {
                            if ( $index_sources_tax_val && isset( $index_new['tax'] ) ) {
                                $index_new['tax']['fields'][$index_sources_tax_name]['value'] = '1';
                            }
                        }
                    }

                    if ( isset( $common_opts['index_sources_meta'] ) && is_array( $common_opts['index_sources_meta'] ) ) {
                        foreach ( $common_opts['index_sources_meta'] as $index_sources_meta_name => $index_sources_meta_val ) {
                            if ( $index_sources_meta_val && isset( $index_new['meta'] ) ) {
                                $index_new['meta']['fields'][$index_sources_meta_name]['value'] = '1';
                            }
                        }
                    }

                    $common_opts['index_sources'] = $index_new;
            ...
            if ( version_compare( $current_version, '3.47', '<' ) ) {
                ...
                if ( $settings ) {
                    ...
                    foreach( $settings as $search_instance_num => $search_instance_settings ) {
                        ...
                        $new_result_settings = array();
                        $new_quick_filters_settings = array();
                        ...
                        if ( isset( $search_instance_settings['filters'] ) ) {
                            ...
                            foreach ($search_instance_settings['filters'] as $filter_num => $filter_settings) {
                                ...
                                if ( empty( $new_result_settings ) ) {

                                    $new_result_settings = $filter_settings;

                                    // update add to cart option
                                    if ( isset( $new_result_settings['show_cart'] ) && $new_result_settings['show_cart'] === 'quantity' ) {
                                        $new_result_settings['show_cart_quantity'] = 'true';
                                        $new_result_settings['show_cart'] = 'true';
                                    } else {
                                        $new_result_settings['show_cart_quantity'] = 'false';
                                    }

                                    // update stock status option
                                    if ( isset( $new_result_settings['show_stock'] ) && $new_result_settings['show_stock'] === 'quantity' ) {
                                        $new_result_settings['show_stock_quantity'] = 'true';
                                        $new_result_settings['show_stock'] = 'true';
                                    } else {
                                        $new_result_settings['show_stock_quantity'] = 'false';
                                    }
                                ...
                                $new_quick_filters_settings[$filter_num] = array(
                                    'item_name' => $filter_settings['filter_name'],
                                    'filter_type' => 'simple',
                                    'filter_by' => 'product_cat',
                                    'max_number' => 0,
                                    'only_parents' => 'true',
                                    'hide_empty' => 'true',
                                    'adv_filters' => $filter_settings['adv_filters'],
                                );

                            }

                            unset($settings[$search_instance_num]['filters']);
                            unset($search_instance_settings['filters']);

                        }

                        if ( ! empty( $new_quick_filters_settings ) ) {
                            $settings[$search_instance_num]['quick_filters'] = $new_quick_filters_settings;
                            $search_instance_settings['quick_filters'] = $new_quick_filters_settings;
                        }

                        if ( ! empty( $new_result_settings ) ) {
                            $settings[$search_instance_num] = array_merge( $new_result_settings, $search_instance_settings );
                        }
