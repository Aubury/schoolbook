...
// Чтение JSON файла
if (file_exists('TEMP/ExchBuxData.json')) {
    $json_data = file_get_contents('TEMP/ExchBuxData.json');
...
$woocommerce = new Client(
    ...
    [
        ...
        'timeout' => 60, // Увеличьте время ожидания // Force Basic Authentication as query string true and using under HTTPS
    ]
);
 ...
 **************/
 ...
 $imagesDirectoryURL  = home_url('/public_html/IMAGES/');
 ...
 ini_set('error_log', __DIR__ . '/public_html/ERROR_LOG/error_log.txt');
...
file_put_contents(__DIR__ . '/public_html/cron-log.txt', date('Y-m-d H:i:s') . " - Cron запущен\n", FILE_APPEND);

 
 if (!is_writable($ftpDirectory)) {
     die("Директория $ftpDirectory недоступна для записи.");
 }
 if (!is_writable(dirname($stateFile))) {
     die("Директория для файла состояния недоступна для записи.");
 }
 ...
 function loadProductsFile ($data): void
 {
     ...
     foreach ($data as $key => $product) {
         try {
             ...
             file_put_contents($file, 'Товар id = ' . intval($ID) . ' ', FILE_APPEND);
             ...
             $message = "{$key}) - Ошибка для товара '{$product['Name']}', UniID: {$product['UniID']}: {$e->getMessage()} - line: {$e->getLine()} \n";
             $log_message = '[' . date('d-m-Y H:i:s') . '] ' . $message . PHP_EOL;
             file_put_contents($file, $log_message, FILE_APPEND);
             flush();
         }
